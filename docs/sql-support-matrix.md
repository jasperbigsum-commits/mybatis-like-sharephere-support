...
| `SELECT *` / `SELECT t.*` | Supported with caveats | Single-table queries and single aliased derived-table queries may use bare `*`; the rewriter qualifies it as `table.*` or `alias.*` when encrypted projections are injected. Multi-table queries must use explicit table wildcards such as `table.*` or `alias.*` so encrypted projections can stay ahead of the wildcard without being overwritten. |
...
| business range query | `where phone > ?` / `between` | redesign query or add a safe business index field; only technical cursor-style single-sided comparisons are supported |
